...
///////////////////////////////////////
     ...
     * - `{void}` `put({string} key, {*} value)` вЂ” Puts a new key-value pair into the cache.
...
///////////////////////////////////
     ...
     * **Methods**
     *
     * - `then(successCallback, errorCallback)` вЂ“ regardless of when the promise was or will be resolved
    ...
    function $RouteProvider(){
         ...
         *    Object properties:
         *
         *    - `controller` вЂ“ `{(string|function()=}` вЂ“ Controller fn that should be associated with newly
         *      created scope or the name of a {@link angular.Module#controller registered controller}
         *      if passed as a string.
         *    - `template` вЂ“ `{string=}` вЂ“  html template as a string that should be used by
         *      {@link ng.directive:ngView ngView} or
         *      {@link ng.directive:ngInclude ngInclude} directives.
         *      this property takes precedence over `templateUrl`.
         *    - `templateUrl` вЂ“ `{string=}` вЂ“ path to an html template that should be used by
         ...
         *
         *      - `key` вЂ“ `{string}`: a name of a dependency to be injected into the controller.
         *      - `factory` - `{string|function}`: If `string` then it is an alias for a service.
         *        Otherwise if function, then it is {@link api/AUTO.$injector#invoke injected}
         *        and the return value is treated as the dependency. If the result is a promise, it is resolved
         *        before its value is injected into the controller.
         *
         *    - `redirectTo` вЂ“ {(string|function())=} вЂ“ value to update
    ...
    function $HttpProvider() {
        ...
        this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
            function($httpBackend, $browser, $cacheFactory, $rootScope, $q, $injector) {
                 ...
                 * injected with dependencies (if specified) and returns the interceptor  вЂ” a function that
                 ...
                 *    - **params** вЂ“ `{Object.<string|Object>}` вЂ“ Map of strings or objects which will be turned to
                 *      `?key1=value1&key2=value2` after the url. If the value is not a string, it will be JSONified.
                 *    - **data** вЂ“ `{string|Object}` вЂ“ Data to be sent as the request message data.
                 *    - **headers** вЂ“ `{Object}` вЂ“ Map of strings representing HTTP headers to send to the server.
                 *    - **transformRequest** вЂ“ `{function(data, headersGetter)|Array.<function(data, headersGetter)>}` вЂ“
                 *      transform function or an array of such functions. The transform function takes the http
                 *      request body and headers and returns its transformed (typically serialized) version.
                 *    - **transformResponse** вЂ“ `{function(data, headersGetter)|Array.<function(data, headersGetter)>}` вЂ“
                 *      transform function or an array of such functions. The transform function takes the http
                 ...
                 *      caching.
                 *    - **timeout** вЂ“ `{number}` вЂ“ timeout in milliseconds.
                 ...
                 *
                 *   - **data** вЂ“ `{string|Object}` вЂ“ The response body transformed with the transform functions.
     ...
     * @example
     ...
     Phone only <input ng-model="search.phone"ГҐ><br>
...
// ng-src, ng-href are interpolated
     ...
     *  forms, where:
     *
     *  - keys are validation tokens (error names) вЂ” such as `REQUIRED`, `URL` or `EMAIL`),
...
//asks for $scope to fool the BC controller module
     ...
     *
     * * Model вЂ” The Model is data in scope properties; scopes are attached to the DOM.
     * * View вЂ” The template (HTML with data bindings) is rendered into the View.
     * * Controller вЂ” The `ngController` directive specifies a Controller class; the class has
     ...
     *
     *   * `$index` вЂ“ `{number}` вЂ“ iterator offset of the repeated element (0..length-1)
     *   * `$first` вЂ“ `{boolean}` вЂ“ true if the repeated element is first in the iterator.
     *   * `$middle` вЂ“ `{boolean}` вЂ“ true if the repeated element is between the first and last in the iterator.
     *   * `$last` вЂ“ `{boolean}` вЂ“ true if the repeated element is last in the iterator.
     *
     *
     * @element ANY
     * @scope
